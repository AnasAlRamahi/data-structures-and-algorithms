...
const sortMeetingsByDay = (arr) => {
  // Solution code here...
  const arrOfDays = ['Saturday', 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'];
  arr.sort((a,b) => {
    if (arrOfDays.indexOf(a.dayOfWeek) < arrOfDays.indexOf(b.dayOfWeek))
    return -1;
  });
  return arr;
};
...
const sortSchedule = (arr) => {
  ...
  arr.sort((a,b) => {
    if (arrOfDays.indexOf(a.dayOfWeek) < arrOfDays.indexOf(b.dayOfWeek))
    return -1;
    else if (arrOfDays.indexOf(a.dayOfWeek) == arrOfDays.indexOf(b.dayOfWeek) && a.start < b.start)
    return -1;
    else if (arrOfDays.indexOf(a.dayOfWeek) == arrOfDays.indexOf(b.dayOfWeek) && a.start == b.start && a.end < b.end)
    return -1;
  });
  return arr;
};
