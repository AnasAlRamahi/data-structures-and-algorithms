...
const validatePin = (pin) => {
  // Solution code here...
  if(pin.toString().match(/\b[0-9]{4}\b/g))
  return true;
  else return false;
};
...
const validateWord = (word) => {
  // Solution code here...
  if(word.toString().match(/\b[A-z]{5,10}\b/g))
  return true;
  else return false;
};
...
const hasNumber = (string) => {
  // Solution code here...
  if(string.toString().match(/\b([A-z]+[0-9]+[A-z]*)\b/g))
  return true;
  else return false;
};
...
const validateEmail = (email) => {
  // Solution code here...
  if(email.toString().match(/^[a-zA-Z0-9.!#$%&’*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/g))
  return true;
  else return false;
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  if(phoneNumber.toString().match(/^\s*(?:\+?(\d[0-9]{1,3}))?[- (]*(\d{3})[- )]*(\d{3})[- ]*(\d{4})(?: *[x/#]{1}(\d+))?\s*$/g))
  return true;
  else return false;
};
